...
class jspquote {
	...
	function get_pqcss($theStyle) {
		If (file_exists(TEMPLATEPATH . '/jspullquotes.css')) {
			// echo("<!-- file_exists(TEMPLATEPATH . '/jspullquotes.css') -->");
...
/* // alt version that didn't work because 
		$optionstring = '	<script type="text/javascript">
		var arrOptions = new Array("';
		foreach ($options as $key => $value) {
			$optionstring .= $value . '", "';
		}
		$optionstring = substr($optionstring,0,$optionstring.length-4);
		$optionstring .= '");
...
// Add the configuration screen to the Presentation menu in wp-admin
	...
	function adminhead() {
		global $version;
?>
	<!-- JavaScript Pull-Quotes plugin v<?php echo($version) ?> --><script src="<?php echo(get_bloginfo('wpurl')); ?>/wp-content/plugins/jspullquotes/preview.js" type="text/javascript" language="JavaScript" charset="utf-8"></script><!-- end pull-quote additions -->
<?php
	}
...
<div class="wrap">
	...
	<form action="themes.php?page=jspullquotes.php" method="post">
		<div style="width: 10em; float: right; border: 1px black solid; margin: 0 0 1em 1em; padding: 0.75em; background-color: #FDFBDE; text-size: 1.2em;">
			<a href="templates.php?file=<?php echo(substr(jspquote::get_pqcss($jspq_options['style_name']),strlen(get_bloginfo('wpurl')))); ?> "\"><?php _e('Edit the currently active stylesheet','jspullquotes') ?>: <?php echo($jspq_options['style_name']); ?></a>
		</div>
		<fieldset name="basic">
...
// need to add JavaScript into second parameter below to check if style is file or directory.
?>			</select> (<a href="#" onclick="previewpop('<?php echo($stylePath); ?>','jspullquotes-core.css',document.getElementById('style_name').value); return false;" title="<?php _e('show a preview of the selected style in a pop-up window','jspullquotes') ?>"><?php _e('preview style','jspullquotes') ?></a>)</p>
<?php	}
?>
