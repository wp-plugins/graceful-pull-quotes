...
// This adds the needed links to the <head> of each blog page
	function wphead() {
		...
		echo '
<!-- added by JavaScript Pull-Quotes plugin - http://www.striderweb.com/nerdaphernalia/features/wp-javascript-pull-quotes/ -->' . "\n";
		echo '	<link rel="stylesheet" href="' . get_bloginfo('wpurl') . '/wp-content/plugins/jspullquotes/jspullquotes-core.css" type="text/css" />' . "\n";
		
		if ($options['style_name'] == null || (file_exists(dirname(__FILE__) . '/styles/' . $options['style_name']) == false)) $options['style_name'] = 'default.css';
		echo '	<link rel="stylesheet" href="' . get_bloginfo('wpurl') . '/wp-content/plugins/jspullquotes/styles/' . $options['style_name'] . '" type="text/css" />' . "\n";
...
<div class="wrap">
	...
	<form action="themes.php?page=jspullquotes.php" method="post">
		<div style="width: 10em; float: right; border: 1px black solid; margin: 0 0 1em 1em; padding: 0.75em; background-color: #FDFBDE; text-size: 1.2em;">
			<a href="templates.php?file=/wp-content/plugins/jspullquotes/styles/<?php echo($jspq_options['style_name']); ?> "\">Edit the currently active stylesheet: <?php echo($jspq_options['style_name']); ?></a>
		</div>
		<fieldset name="basic">
		<h3>Basic Options</h3>
			<p><input type="checkbox" name="jspullquotes_options[skip_links]" id="skip_links" value="true"<?php echo(jspquote::checkflag($jspq_options,'skip_links')); ?> /> <label for="skip_links">Remove link information from pull-quotes</label> <strong>(recommended)</strong></p>
<?php
		if ($arrStyles[1] != null) {
		?>
			<p><label for="style_name">Pull-quote style: </label><select name="jspullquotes_options[style_name]" id="style_name">
<?php
			$i = 0;
			while ($arrStyles[$i] != null) {
				...
				echo "				<option value=\"" . $style . "\"" . jspquote::checkcombo($jspq_options,'style_name',$style,$deftag) . ">" . ucfirst(basename($style,'.css')) . "</option>\n";
				$i++;
			}
?>			</select></p>
<?php	}
?>
			...
			<p><input type="checkbox" name="jspullquotes_options[alt_text]" id="alt_text" value="true"<?php echo(jspquote::checkflag($jspq_options,'alt_text')); ?> /> <label for="alt_text">Use alternate text if available</label> (<a href="../wp-content/plugins/jspullquotes/help/alt-text-info.htm">how?</a>)</p>
		</fieldset>
...
<!--			<p><strike><input type="checkbox" name="jspullquotes_options[use_beta]" id="use_beta" value="true"<?php echo(jspquote::checkflag($jspq_options,'use_beta')); ?> /> <label for="use_beta">(BETA) Use pull-quote button in rich-text post editor</label></strike></p> -->
			...
			<p>Note: changing any of the following may require you to update your CSS file.</p>
			...
			<p><label for="quote_class_alt">CSS class selector for alt-side pull-quote:</label> <input type="text" name="jspullquotes_options[quote_class_alt]" id="quote_class_alt" value="<?php echo(jspquote::checktext($jspq_options,'quote_class_alt','pullquote pqRight')); ?>"/></p>
		</fieldset>
		<div class="submit">
