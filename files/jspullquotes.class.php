...
// This adds the needed links to the <head> of each blog page
	function wphead() {
		...
		echo '
<!-- JavaScript Pull-Quotes plugin v' . $jspq_version[0] . '
...
// these three functions are used by the options page to display set options in the form controls when the page is opened
	...
	function checkcombo( $options, $optname, $thisopt, $optdefault= false ) { // for dropdowns
...
<?php	}
		$cmbpicksides = '<select name="jspullquotes_options[def_side]" id="def_side">
				<option value="left"' . jspullquotes::checkcombo($jspq_options,'def_side','left',true) . '>' . __('left','jspq') . '</option>
				<option value="right"' . jspullquotes::checkcombo($jspq_options,'def_side','right') . '>' . __('right','jspq') . '</option>
			</select>';
?>
			<p><label for="def_side"><?php echo(sprintf(__('Display quotes on %s side','jspq'), $cmbpicksides)); ?></label></p>
			...
			<p><input type="checkbox" name="jspullquotes_options[alt_text]" id="alt_text" value="true"<?php echo(jspullquotes::checkflag($jspq_options,'alt_text')); ?> /> <label for="alt_text"><?php _e('Use alternate text if available','jspq'); ?></label> (<a href="<?php echo(get_bloginfo('wpurl') . '/' . JSPQPATH) ?>/files/help/alt-text-info.<?php _e('en_US','jspq'); ?>.htm"><?php _e('how?','jspq'); ?></a>)</p>
		</fieldset>
...
<?php
		$cmbq_container = '<select name="jspullquotes_options[q_container]" id="q_container">
				<option value="blockquote"' . jspullquotes::checkcombo($jspq_options,'q_container','blockquote',true) . '>&lt;blockquote&gt;</option>
				<option value="div"' . jspullquotes::checkcombo($jspq_options,'q_container','div') . '>&lt;div&gt;</option>
				</select>';
?>
			<p><label for="def_side"><?php echo(sprintf(__('Contain pull-quote in an HTML %s','jspq'), $cmbq_container)); ?></label></p>
			...
			<p><input type="checkbox" name="jspullquotes_options[omit_styles]" id="omit_styles" value="true"<?php echo(jspullquotes::checkflag($jspq_options,'omit_styles')); ?> /> <label for="omit_styles"><?php _e('Do not link CSS','jspq'); ?></label> <?php _e('(Check this if you prefer to manually put your pull-quote styles elsewhere)','jspq'); ?></p> 
	...
	JavaScript Pull-Quotes v<?php echo($jspq_version[0]) ?> -- <a href="http://striderweb.com/nerdaphernalia/features/wp-javascript-pull-quotes/"><?php _e('home page','jspq'); ?></a>
</div><!-- end of wrap -->
	<?php
	}
